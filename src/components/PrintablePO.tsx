...
export default function PrintablePO({ po, entities }: { po: any, entities: any[] }) {
    useEffect(() => {
        const timer = setTimeout(() => window.print(), 500);
        return () => clearTimeout(timer);
    }, []);

    if (!po) return <div className="p-10 font-bold text-red-500">Document failed to load.</div>;

    const vendor = entities.find(e => e.id === po.entity_id);
    ...
    const totalGrossPreTax = (po.transaction_lines || []).reduce((sum: number, m: any) => sum + getLineNetPreTax(m), 0);
    const totalTax = (po.transaction_lines || []).reduce((sum: number, m: any) => sum + getLineTaxAmt(m), 0);
    const subtotalWithTax = totalGrossPreTax + totalTax;
    const overallDiscountAmt = subtotalWithTax * ((Number(po.overall_discount) || 0) / 100);
    const shippingAmt = Number(po.shipping_amount) || 0;
    ...
    return (
        <div className="bg-white text-slate-900 min-h-screen p-8 md:p-12 max-w-5xl mx-auto font-sans">
            {/* SARGENT FIX: Replaced flex with strict 2-column CSS Grid to prevent overlap */}
            <div className="grid grid-cols-2 gap-8 items-start border-b-2 border-slate-900 pb-8 mb-8">
                ...
                <div className="text-right flex flex-col items-end">
                    ...
                    <div className="space-y-1 text-right">
                        <p className="text-sm text-slate-500"><span className="font-bold text-slate-800 w-24 inline-block">PO #:</span> PO-{po.transaction_number || po.id.split('-')[0].toUpperCase()}</p>
                        <p className="text-sm text-slate-500"><span className="font-bold text-slate-800 w-24 inline-block">Date:</span> {new Date(po.transaction_date).toLocaleDateString()}</p>
                        <p className="text-sm text-slate-500"><span className="font-bold text-slate-800 w-24 inline-block">Status:</span>
                            <span className="font-black text-slate-800 uppercase">{po.status.replace('_', ' ')}</span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-12 mb-10">
                <div>
                    <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2 border-b border-slate-200 pb-2">Vendor / Remit To</h3>
                    {vendor ? (
                        <>
                            <p className="text-lg font-bold text-slate-800">{vendor.name}</p>
                            <p className="text-sm text-slate-500">{vendor.email || 'No email on file'}</p>
                            {vendor.phone && <p className="text-sm text-slate-500">{vendor.phone}</p>}
                            {vendor.billing_address && <p className="text-sm text-slate-500 mt-2 whitespace-pre-line">{vendor.billing_address}</p>}
                        </>
                    ) : (
                        <p className="text-sm text-slate-500 italic">No Vendor Linked</p>
                    )}
                </div>
                <div>
                    <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2 border-b border-slate-200 pb-2">Shipping Details</h3>
                    <p className="text-sm text-slate-500 mt-2"><span className="font-bold text-slate-800">Ship To:</span> TankTech Asia HQ</p>
                    <p className="text-sm text-slate-500 mt-1"><span className="font-bold text-slate-800">Terms:</span> Net 30</p>
                </div>
            </div>
            ...
            <table className="w-full text-left text-sm mb-12">
                ...
                <tbody className="divide-y divide-slate-200">
                    {po.transaction_lines?.map((line: any) => (
                        <tr key={line.id}>
            ...
            <div className="flex justify-end mb-16">
                <div className="w-80 space-y-3">
                    ...
                    {Number(po.overall_discount) > 0 && (
                        <div className="flex justify-between text-sm text-slate-600">
                            <span>Overall Discount ({Number(po.overall_discount || 0)}%)</span>
            ...
            <div className="mt-24 pt-8 border-t border-slate-200 text-center">
                <p className="text-xs font-bold text-slate-600 uppercase tracking-widest">Authorized By Procurement Dept.</p>
            </div>
        </div>
    );
}